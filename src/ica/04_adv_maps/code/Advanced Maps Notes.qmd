...
```{r setup}
# Note to me : This r setup is a name of the code chunk -- setup is codechunk name and r means r code chunk
...
mn_cities <- st_read("../Data/Minnesota_Population_Centers/city_and_township_population_centers.shp")
mn_water <- st_read("../Data/Lakes_and_Rivers_Open_Water_Features/LakesAndRivers.shp")
```
...
# Ths transform_and_plot is like a normal function in a any Programming Language 
...
ggplot(nc) +
    geom_sf() +
    theme_classic() +
    labs(title = "NAD27 (Original)") # Just givin g the title 
...
It won't be as accurate because the point does not fit the NAD27 datum CRS and since NAD27 is for all of America and North Carolina is in US, the view would be distorted. 
...
```{r}
st_crs(mn_counties) # mn_counties CRS is different  EPSG:4326 
```
...
ggplot() + # plot frame
    ...
    ggthemes::theme_map() # removes x and y 
